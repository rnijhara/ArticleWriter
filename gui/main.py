...
class Main(QtGui.QMainWindow):
    ...
    def initMenubar(self):
        ...
        helpAction = QtGui.QAction(QtGui.QIcon("icons/help.png"), "How to use?", self)
        helpAction.triggered.connect(helpview.HelpViewer(self).show)
        help.addAction(helpAction)

    def initUI(self):

        self.text = mytextedit.MyTextEdit()
        font = QtGui.QFont()
        font.setPointSize(12)
        self.setFont(font)
        self.text.append("Hello World")
        # self.text.textChanged.connect(self.printa)
        self.text.connect(self.text, SIGNAL("ctrlSpacePressed"), self.ctrl_space)
        self.text.textChanged.connect(self.hide_list)
        self.initToolbar()
        self.initFormatbar()
        self.initMenubar()
        ...
        self.ls = mylistwidget.MyListWidget(self.text)
        self.ls.resize(120, 120)
        self.ls.hide()
        self.ls.connect(self.ls, SIGNAL("ctrlEPressed"), self.ctrl_e)
        ...
        self.setGeometry(120, 30, 1030, 700)
        if self.filename == "":
            self.setWindowTitle("ArticleWriter - Untitled Document")
        else:
            self.setWindowTitle("ArticleWriter - {}".format(self.filename))
    ...
    def ctrl_space(self):
        cursor = self.text.cursorRect()
        self.ls.show()
        self.ls.move(cursor.x(), cursor.y() + 23)
        self.ls.clear()
        self.ls.setFocus()
        plain_text = str(self.text.toPlainText()).rstrip(' \t\n\r')
        words = plain_text.split(" ")
        n = len(words)
        try:
            if n >= 3:
                context = (words[-3].lower(), words[-2].lower(), words[-1].lower())
                predictions = self.ngram_model.predict(context)
            elif n == 2:
                context = (words[-2].lower(), words[-1].lower())
                predictions = self.ngram_model.backoff.predict(context)
            elif n == 1:
                context = (words[-1].lower(),)
                predictions = self.ngram_model.backoff.backoff.predict(context)
            else:
                context = ()
                predictions = self.ngram_model.backoff.backoff.backoff.predict(context)

            for pred in predictions:
                self.ls.addItem(pred)
        except (AttributeError, UnboundLocalError):
            self.ls.hide()
            self.text.setFocus()

    def ctrl_e(self):
        self.text.insertPlainText(" " + self.ls.currentItem().text())
        self.ls.hide()
        self.text.setFocus()
    ...
    def initPredictor(self):
        self.ppthread = PreparePredictor()
        self.connect(self.ppthread, SIGNAL("addModel(PyQt_PyObject)"), self.addModel)
        self.ppthread.start()
        self.msg = QLabel("Initialising Next Word Predictor ")
        self.statusbar.addPermanentWidget(self.msg)

    def addModel(self, lm):
        self.ngram_model = lm
        self.statusbar.removeWidget(self.msg)
        self.msg = QLabel('Next Word Predictor is ready. You can get predictions by pressing Ctrl+Space ')
        self.statusbar.addPermanentWidget(self.msg)
        self.waitThread = WaitThread()
        self.connect(self.waitThread, SIGNAL("finished()"), self.removeMsg)
...
class PreparePredictor(QThread):
    def __init__(self):
        QThread.__init__(self)

    def run(self):
        lm = cPickle.load(open("../ngram.bin", "rb"))
        self.emit(SIGNAL('addModel(PyQt_PyObject)'), lm)
